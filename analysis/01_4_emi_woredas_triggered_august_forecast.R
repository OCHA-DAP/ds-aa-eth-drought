...
somali_csv_blob <- storage_download(container, 
                                    src = "ds-aa-eth-drought/exploration/AA Dashboard - Somali Region, OND Season - August.csv", 
                                    dest = som_temp_file)
somali_region_woredas <- read_csv(som_temp_file)
oro_temp_file <- tempfile(fileext = ".csv")
oromia_csv_blob <- storage_download(container, 
                                    src = "ds-aa-eth-drought/exploration/AA Dashboard - Southern Oromia Region, OND Season - August.csv", 
                                    dest = oro_temp_file)
...
ond_woredas_triggered |>
  filter(Severity == "Moderate" & `Triggered?` == "Yes") |>
  ...
  labs(title = "Moderate Drought Trigger Status for OND 2024 in Ethiopia",
       subtitle = "Based on EMI August Forecast with a 2-Month Lead Time")
  
# severe areas triggered
ond_woredas_triggered |>
  filter(Severity == "Severe" & `Triggered?` == "Yes") |>
  merge(eth_adm3_codab, by.x = "Woreda", by.y = "admin3Name_en", all.y = T) |>
  ggplot() +
  geom_sf(aes(fill = `Triggered?`, geometry = geom)) + 
  geom_sf(data = eth_adm1_codab, fill = NA, linewidth = 0.8, color = "black") +
...
ond_woredas_triggered |>
  ...
  mutate(Severity = ifelse(is.na(Severity) | Severity == "No", NA, Severity)) |>
  ggplot() + 
  ...
  labs(title = "OND 2024 Forecast for Ethiopia",
       subtitle = "Evaluated Using the August EMI forecast with a 2-Month Lead Time",
